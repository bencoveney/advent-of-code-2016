var data = `rhamaeovmbheijj[hkwbkqzlcscwjkyjulk]ajsxfuemamuqcjccbc
...
var abbaRegex = /[a-z]*(?:([a-z])([a-z])\2\1)[a-z]*/;
var partsRegex = /([a-z]*)\[([a-z]*)\]([a-z]*)/; 

function testAbba(string) {
  var match = abbaRegex.exec(string);
...
data
  .split("\n")
  .map((address) => {
    var firstBreakdown = address.split("[");
    var result = {
      good: [firstBreakdown.shift()],
      bad: []
    };
    firstBreakdown.forEach((addressPart) => {
      var partParts = addressPart.split("]");
      result.good.push(partParts[1]);
      result.bad.push(partParts[0]);
    })
    return result;
  })
  .filter((addressParts) => {
    return !addressParts.bad.some(testAbba) && addressParts.good.some(testAbba);
  })
  .length
